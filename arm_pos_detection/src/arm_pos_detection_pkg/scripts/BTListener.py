...
class SubscribeAndPublish:
    def __init__(self, block, block2, upperArmLen, lowerArmLen):
        ...
        self.serialSubscriber = rospy.Subscriber('Str_from_BT', String, self.callback)
        ...
        self.upperArmLen = upperArmLen
        self.lowerArmLen = lowerArmLen

        self.f = None

        self.characterWinkel1 = 0.0
        self.characterWinkel2 = 0.0

        self.recordingStat = True
        
        self.characterWinkelSpeicher1 = []
        self.characterWinkelSpeicher2 = []

        self.shoulder_q0_speicher = []
        self.shoulder_q1_speicher = []
        self.shoulder_q2_speicher = []
        self.shoulder_q3_speicher = []

        self.elbow_q0_speicher = []
        self.elbow_q1_speicher = []
        self.elbow_q2_speicher = []
        self.elbow_q3_speicher = []

        self.wrist_coord_x_speicher = []
        self.wrist_coord_y_speicher = []
        self.wrist_coord_z_speicher = []


    def stopRecording(self):
        #print("Speicher1: ", self.characterWinkelSpeicher1)
        #print("Speicher2: ", self.characterWinkelSpeicher2)
        self.f.write("WINKEL_OBERARM: " + str(np.mean(self.characterWinkelSpeicher1)) + "\n")
        self.f.write("WINKEL_UNTEARM: " + str(np.mean(self.characterWinkelSpeicher2)) + "\n")
        self.f.write("QUATERNION_SHOULDER: " + str(np.mean(self.shoulder_q0_speicher)) + " " + str(np.mean(self.shoulder_q1_speicher)) + " " + str(np.mean(self.shoulder_q2_speicher)) + " " +str(np.mean(self.shoulder_q3_speicher)) + "\n")
        self.f.write("QUATERNION_ELBOW: " + str(np.mean(self.elbow_q0_speicher)) + " " + str(np.mean(self.elbow_q1_speicher)) + " " + str(np.mean(self.elbow_q2_speicher)) + " " +str(np.mean(self.elbow_q3_speicher)) + "\n")
        self.f.write("WRIST_COORDINATE: " + str(np.mean(self.wrist_coord_x_speicher)) + " " + str(np.mean(self.wrist_coord_y_speicher)) + " " + str(np.mean(self.wrist_coord_z_speicher)) + "\n" + "\n")
        self.recordingStat = False
        self.characterWinkelSpeicher1 = []
        self.characterWinkelSpeicher2 = []
    ...
    def callbackKeyBoard(self, data):
        ...
        print(data)
        print(type(data))
        if data.data == 65:  #up create a file
            self.calibrated = True
            caliTimeStamp = rospy.Time.now()
            localTime = time.localtime(int(caliTimeStamp.secs))
            docuName = time.strftime("%Y-%m-%d--%H:%M:%S", localTime)
            print(docuName)
            #print(str(caliTimeStamp.secs)+'.'+str(caliTimeStamp.nsecs))
            self.f = open('/home/ubuntu/data/' + docuName + 'txt', "w")
            self.f.write("Sampling Time: " + docuName + "\n" + "\n")

        elif data.data == 66: #down: stop record and close file
            self.f.close()

        elif data.data == 67: #right: start recording one gesture
            self.recordingStat = True

            timer = threading.Timer(4, self.stopRecording)
            timer.start()
    ...
    def callback(self, data):
        ...
        acc = np.array([floatList[1], -floatList[0], -floatList[2]])
        gyro = np.array([-floatList[4], floatList[3], floatList[5]])
        mag = np.array([floatList[6], floatList[7],floatList[8]])
        ...
        magGaussRaw = np.matmul(mag - self.wireframe.mag_b, self.wireframe.mag_Ainv)
        ...
        upperArmUnitVectorLocal = np.array([0.0, 1.0, 0.0, 0.0])

        self.elbowBroadcaster.sendTransform(translationShoulder, rotationShoulder, rospy.Time.now(), 'upper_arm', '/base')

        upperArmUnitVectorGlobal = quatMultiplication(quatAfterCali, quatMultiplication(upperArmUnitVectorLocal, quatAfterCaliReverse))

        #print("upperArmUnitVectorGlobal: ", upperArmUnitVectorGlobal)

        self.characterWinkel1 = np.arccos(-upperArmUnitVectorGlobal[3]) * 180 / np.pi
        #print("characterWinkel1: ", characterWinkel1)


         
        elbowCoord = quatMultiplication(quatAfterCali, quatMultiplication(self.upperArmLen * upperArmUnitVectorLocal, quatAfterCaliReverse))
        #print("elbowCoord: ", elbowCoord)
        ...
        acc2 = np.array([floatList[1+9], -floatList[0+9], -floatList[2+9]])
        gyro2 = np.array([-floatList[4+9], floatList[3+9], floatList[5+9]])
        #print("gyro2: ", gyro2)
        mag2 = np.array([floatList[6+9], floatList[7+9],floatList[8+9]])
        ...
        magGaussRaw2 = np.matmul(mag2 - self.wireframe2.mag_b, self.wireframe2.mag_Ainv)
        ...
        quatAfterCaliReverse2 = quatReverse(quatAfterCali2)
        #print("2: ", quatAfterCali2)
        ...
        #print("3: ", quatAfterCali2)
        quatAfterCaliReverse = quatReverse(quatAfterCali)
        quatAfterCali2TFBr = quatMultiplication(quatAfterCaliReverse, quatAfterCali2)

        q0_elbow, q1_elbow, q2_elbow, q3_elbow = quatAfterCali2TFBr

        
        quatAfterCali2TFBrReverse2 = quatReverse(quatAfterCali2TFBr)
        ...
        lowerArmUnitVectorLocal = np.array([0.0, 1.0, 0.0, 0.0])


        self.elbowBroadcaster.sendTransform(translationElbow, rotationElbow, rospy.Time.now(), 'lower_arm', 'upper_arm')


        lowerArmUnitVectorGlobal = quatMultiplication(quatAfterCali2, quatMultiplication(lowerArmUnitVectorLocal, quatAfterCaliReverse2))

        #print("lowerArmUnitVectorGlobal: ", lowerArmUnitVectorGlobal)

        self.characterWinkel2 = np.arccos(lowerArmUnitVectorGlobal[1] * upperArmUnitVectorGlobal[1] + lowerArmUnitVectorGlobal[2] * upperArmUnitVectorGlobal[2] + lowerArmUnitVectorGlobal[3] * upperArmUnitVectorGlobal[3]) * 180 / np.pi
        #print("characterWinkel2: ", characterWinkel2)


        wristCoord = elbowCoord + quatMultiplication(quatAfterCali2, quatMultiplication(self.upperArmLen * upperArmUnitVectorLocal, quatAfterCaliReverse2))
        #print("wristCoord: ", wristCoord)

        if self.recordingStat:
            self.characterWinkelSpeicher1.append(self.characterWinkel1)
            self.characterWinkelSpeicher2.append(self.characterWinkel2)

            self.shoulder_q0_speicher.append(q0_shoulder)
            self.shoulder_q1_speicher.append(q1_shoulder)
            self.shoulder_q2_speicher.append(q2_shoulder)
            self.shoulder_q3_speicher.append(q3_shoulder)

            self.elbow_q0_speicher.append(quatAfterCali2[0])
            self.elbow_q1_speicher.append(quatAfterCali2[1])
            self.elbow_q2_speicher.append(quatAfterCali2[2])
            self.elbow_q3_speicher.append(quatAfterCali2[3])

            self.wrist_coord_x_speicher.append(wristCoord[1])
            self.wrist_coord_y_speicher.append(wristCoord[2])
            self.wrist_coord_z_speicher.append(wristCoord[3])
...
if __name__ == '__main__':
    ...
    SubscribeAndPublish(block,block2, upperArmLen, lowerArmLen)
